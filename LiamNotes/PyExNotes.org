...
** XlsxWriter
This is similar to openpyxl, but focused on creating excel files.
The documentation is a bit better organised.
So this might be a better option if all you need is an export
...
The output of this script is an xlsx file, with some prefilled cells, zoomed to 200%
...
# write the data into the workbook, with styles
...
# Autofit the table, set zoom and close
worksheet.autofit()
...
**** Output file
The output file for this script is a formatted table. 
...
**** Output file
The output file for the above is a formatted table, with a chosen table style.
...
**** Output file
A table where margins over 75c are highlighted.
